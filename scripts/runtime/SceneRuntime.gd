...
func _build_actor(data: Dictionary) -> void:
    ...
    var anim_config := data.get("animation", {})
    var anim_fps := 8.0
    var colors: Array = [Color("#f7d35d"), Color("#f39c12"), Color("#f7e29a")]
    if typeof(anim_config) == TYPE_DICTIONARY:
        anim_fps = float(anim_config.get("fps", 8.0))
        var source_colors := anim_config.get("colors", [])
        if typeof(source_colors) == TYPE_ARRAY and not source_colors.is_empty():
            colors.clear()
